...
state initial_state(int32_t frame){
    state st;
    st.frame = frame;
    for(int i=0;i<MAX_PLAYERS;i++) st.scores[i] = 0;
    int legion_size = 0;
    while(legion_size*legion_size<N_BALLS) legion_size++;
    for(int i=0;i<N_BALLS;i++){
        ...
        st.balls[i].v_y = 0;
        st.balls[i].player = -1;
        for(int r=0;r<N_BUTTONS;r++) st.balls[i].button_states[r] = 0;
    }
}
...
state advance_state(const state *ini, event *events, int n_events){
    ...
    for(int i=0;i<N_BALLS;i++){
        ...
        if(ba->player>=0){
            ...
            if(ba->p_x>=4*ARENA_SIZE/5) ba->v_x-= ACCEL;
            if(ba->p_x<=ARENA_SIZE/5) ba->v_x+= ACCEL;
            if(ba->p_y>=4*ARENA_SIZE/5) ba->v_y-= ACCEL;
            if(ba->p_y<=ARENA_SIZE/5) ba->v_y+= ACCEL;
        }
        // Limit speed:
        if(ba->v_x>MAX_SPEED) ba->v_x = MAX_SPEED;
        if(ba->v_x<-MAX_SPEED) ba->v_x = -MAX_SPEED;
        if(ba->v_y>MAX_SPEED) ba->v_y = MAX_SPEED;
        if(ba->v_y<-MAX_SPEED) ba->v_y = -MAX_SPEED;
        // Update position:
        ba->p_x += ba->v_x;
        ba->p_y += ba->v_y;
